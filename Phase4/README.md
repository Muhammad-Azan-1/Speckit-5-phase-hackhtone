...
In Minikube (Kubernetes), the MCP server runs as a separate pod (todo-chatbot-mcp-server), while locally it runs inside the backend thread.

Since you are running locally right now (Phase 4 folder) but want to simulate the Kubernetes behavior where MCP is separate, you need to run it separately.

🚀 How to Run MCP Server Separately (Like in K8s):
Stop the current backend (Ctrl+C).
Enable separate mode: Set DISABLE_EMBEDDED_MCP=true.
