...
- Use the "frontend-design-mcp" skill to create beautiful, modern designs using shadcn/ui components
- Use "frontend-design-tester" to verify the designs look perfect and function correctly
...
- `@specs/ui/` - UI specifications (will be created as part of this feature)

## Clarifications

### Session 2026-01-07

- Q: Which design system should be used for the UI components? → A: Use shadcn/ui design system as specified in frontend CLAUDE.md
- Q: What specific validation rules should be implemented for the forms? → A: Use Zod with min 8 chars for passwords, email format validation, and name length limits
- Q: How should error handling be implemented in the UI? → A: Display inline validation errors, toast notifications for API errors, and appropriate error codes
- Q: Should the frontend-design-mcp skill be used for UI creation? → A: Yes, must use frontend-design-mcp skill for creating UI components
- Q: Should the frontend-design-tester skill be used for verification? → A: Yes, must use frontend-design-tester skill to verify UI components
...
- **FR-003**: System MUST validate form inputs using Zod with appropriate error messages (min 8 chars for passwords, email format validation, name length limits)
- **FR-004**: System MUST display loading states during authentication operations
- **FR-005**: System MUST store JWT tokens securely in the browser after successful authentication using httpOnly cookies when possible or secure localStorage with proper encryption
- **FR-006**: System MUST include JWT tokens in Authorization header for all protected API requests using the format: "Authorization: Bearer <token>"
- **FR-007**: System MUST redirect users to appropriate pages after successful authentication
- **FR-008**: System MUST display clear error messages for authentication failures (inline validation errors, toast notifications for API errors)
...
- **FR-017**: System MUST use shadcn/ui design system components for consistent styling
- **FR-018**: System MUST utilize the "frontend-design-mcp" skill for creating UI components with modern design principles
- **FR-019**: System MUST utilize the "frontend-design-tester" skill to verify UI components function correctly and look perfect
...
- **LoginPage**: The login page component at /login with email and password fields
- **RegisterPage**: The registration page component at /register with email, password, and name fields
- **LoginForm**: Form component with validation, loading states, and error handling
- **RegisterForm**: Form component with validation, loading states, and error handling
- **AuthCard**: Container component with consistent styling for auth forms
- **AuthButtons**: Button components for form submission and navigation
- **AuthInputs**: Input components with proper validation and error display
